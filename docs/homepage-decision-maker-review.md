...
Branch: `feat/homepage-decision-maker` (20 commits on top of `main` 87a431f; not pushed, not deployed)
...
Client cases (`app/components/ClientCases.tsx`). Directly under the hero: a page-wide carousel with one slide per client case, all seven, in the same calm layout: client or sector, a prose result heading (restating the approved headline, no new figures), one sentence, the highlight figures, the attributed quote where one exists, and a link to the full case. The first slide is the Eurofins Genomics process automation and LIMS integration case (7 to 1 data entry points, 75% less order setup time, 50% of capacity freed, quote from Annika Schott). Everything is read from `app/data/work.ts`; nothing is typed twice. All slides are in the server-rendered HTML; there is no auto-advance, arrows on desktop, arrows and dots under the content on phones, and the viewport follows the active slide's height.
